  ...
  <html>
    ...
    <body>
       ...
       <script>
        ...
        <script>
             function alertaError(){
               Swal.fire({
                  icon: 'error',
                  title: 'Oops...!',
                  text: 'Inconveniente actualizando datos'
                }).then(
                   // function() {
                    //    window.location = 'generar_grafico_fecha1.php'
                    //}
        ...
        <script>
             function alertaErrorRangos(){
               Swal.fire({
                  icon: 'error',
                  title: 'Verificar Valores Maximos de Lluvia...!',
                  text: 'Valor Máximo SIN LLUVIA < Valor Máximo LLOVIZNANDO < Valor Máximo LLOVIENDO.....   Valores Máximos de LLUVIA >= 0'
                }).then(
                   // function() {
                    //    window.location = 'generar_grafico_fecha1.php'
                    //}
...
<?php
 ...
 if ((isset($_POST["enviado"])))  // Ingresa a este if si el formulario ha sido enviado..., al ingresar actualiza los datos ingresados en el formulario, en la base de datos.
   ...
   if ($enviado == "S1")
    {
          $temp_max = $_POST["temp_max"];  // aqui se almacenas las variables recibidas del formulario
          $hum_max = $_POST["hum_max"];
          $lluvia1_max = $_POST["lluvia1_max"];
          $lluvia2_max = $_POST["lluvia2_max"];
          $lluvia3_max = $_POST["lluvia3_max"];
          
          if(($lluvia1_max < $lluvia2_max) and ( $lluvia2_max < $lluvia3_max) and ($lluvia1_max >=0)and ($lluvia2_max >=0)and ($lluvia3_max >=0)){
          ...
          $sql1 = "UPDATE datos_maximos set maximo='$temp_max' where id=1";   // se actualiza temperatura maxima
          //echo "sql1...".$sql1;
          ...
          $sql2 = "UPDATE datos_maximos set maximo='$hum_max' where id=2";    // se actualiza humedad maxima
          $result2 = $mysqli->query($sql2);
          
          $sql7 = "UPDATE rango_lluvia set rango='$lluvia1_max' where id=1";  // se actualiza rango maximo sin lluvia
          $result7 = $mysqli->query($sql7);
          $sql8 = "UPDATE rango_lluvia set rango='$lluvia2_max' where id=2";  // se actualiza rango maximo lloviznando
          $result8 = $mysqli->query($sql8);
          $sql9 = "UPDATE rango_lluvia set rango='$lluvia3_max' where id=3";  // se actualiza rango maximo lloviendo
          $result9 = $mysqli->query($sql9);
          
          if (($result1 == 1)and($result2 == 1)and($result7 == 1)and($result8 == 1)and($result9 == 1)){
             $mensaje = "Datos actualizados correctamente";
             echo '<script> alertaExito(); </script>';
          }else{
             echo '<script> alertaError(); </script>';
          }
          }else{
              echo '<script> alertaErrorRangos(); </script>';
          }
...
// CONSULTA RANGO MAXIMOS DE LLUVIA    
$sql6 = "SELECT DISTINCT rango from rango_lluvia ORDER BY rango;"; 
...
// CONSULTA CLASIFICACION DE LLUVIA
$sql5 = "SELECT DISTINCT clasificacion from rango_lluvia ORDER BY rango;"; 
...
?>    
     ...
     <form method=POST action="Consultar_Modificar_Maximos.php">
	     ...
	     <tr>
      		<td bgcolor="#CCEECC" align=center> 
			   	  <font FACE="arial" SIZE=2 color="#000044"> <b>Valor Máximo <?php echo $estados[0]; ?>: </b></font>  
				  </td>	
				  <td bgcolor="#EEEEEE" align=center> 
				  <input type="number" name="lluvia1_max" value="<?php echo $rangos[0]; ?>" required>  
            </td>	
	     </tr>
	     <tr>
      		<td bgcolor="#CCEECC" align=center> 
			   	  <font FACE="arial" SIZE=2 color="#000044"> <b>Valor Máximo <?php echo $estados[1]; ?>: </b></font>  
				  </td>	
				  <td bgcolor="#EEEEEE" align=center> 
				  <input type="number" name="lluvia2_max" value="<?php echo $rangos[1]; ?>" required>  
            </td>	
	     </tr>
	     <tr>
      		<td bgcolor="#CCEECC" align=center> 
			   	  <font FACE="arial" SIZE=2 color="#000044"> <b>Valor Máximo <?php echo $estados[2]; ?>: </b></font>  
				  </td>	
				  <td bgcolor="#EEEEEE" align=center> 
				  <input type="number" name="lluvia3_max" value="<?php echo $rangos[2]; ?>" required>  
            </td>	
	     </tr>
