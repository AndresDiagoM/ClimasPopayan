...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 	Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
  <html>
    <head>
    <title> Datos Promedios Históricos </title>
    </head>
    <body>
       ...
       <script>
            function alertaSinDatos(){
               Swal.fire({
                  icon: 'error',
                  title: 'Oops...',
                  text: 'No se encuentran Datos en este año'
                }).then(
                    function() {
                        window.location = 'generar_grafico_histórico.php'
                    }
                );
            }
        </script>
        ...
        <table width="100%" align=center cellpadding=5 border=0 bgcolor="#FFFFFF">
            ...
            <tr valign="top">
              <td height="20%" align="left" 				
                  ...
                  style="color: #FFFFFF; 
                  font-weight: bold">
                  <font FACE="arial" SIZE=2 color="#000044"> <b><h1>Datos Promedios Históricos</h1></b></font>  
              </td>
              <td height="20%" align="right" 				
                  bgcolor="#FFFFFF" class="_espacio_celdas" 					
                  style="color: #FFFFFF; 
                  font-weight: bold">
                  <img src="img/historicos.png" border=0 width=115 height=115>    
              </td>
            </tr>
          </table>
...
<?php
 ...
 if ((isset($_POST["enviado"])))
   ...
   if ($enviado == "S1")
    {
       $year = $_POST["año"];  // en estas variables se almacenan los datos de fechas recibidos del formulario HTML inicial
       $med = $_POST["medida"];
       ...
       if($med == 'Lluvia'){
           $sql =  "SELECT DISTINCT (estado_lluvia), COUNT(*) as count FROM datos_medidos WHERE (YEAR(fecha)='$year' AND ID_TARJ='$ubi') GROUP BY estado_lluvia;";
        
           $sql1 = "SELECT DISTINCT estado_lluvia from datos_medidos WHERE (YEAR(fecha)='$year' AND ID_TARJ='$ubi') ORDER BY estado_lluvia;";
           ...
           $sql = "SELECT ROUND(AVG($med),2) as count FROM datos_medidos WHERE (YEAR(fecha)='$year' AND ID_TARJ='$ubi') GROUP BY MONTH(fecha) ORDER BY MONTH(fecha)";
            //$sql = "SELECT ROUND(AVG($med),2) as count FROM datos_medidos WHERE (YEAR(fecha)='$year' AND ID_TARJ='$ubi') GROUP BY HOUR(hora) ORDER BY hora";
            
            //$sql1="SELECT DISTINCT hour(hora) from datos_medidos  WHERE fecha = '$year' ORDER BY HOUR(hora) ;";
            $sql1 = "SELECT DISTINCT MONTH(fecha) from datos_medidos WHERE (YEAR(fecha)='$year' AND ID_TARJ='$ubi') ORDER BY MONTH(fecha);";
           ...
           ?>
                            $(function () { 
                                ...
                                $('#container').highcharts({
                                    ...
                                    title: {
                                        text: '<?php echo 'Datos promedio de '.$med.' en '.$UBI[0].' '.  $year ?>'
                                    },
                                		tooltip: {
                                        enabled: false
                                    },
...
<?php
 echo '
      <tr>	
        <form method=POST action="generar_grafico_histórico.php">
...
?>    
     <form method=POST action="generar_grafico_histórico.php">
 	     <tr>	
			...
			<!--	<input type="month" name="fecha_ini" value="" required>  
